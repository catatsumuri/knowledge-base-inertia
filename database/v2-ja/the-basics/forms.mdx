...
<ClientSpecific>
従来の HTML フォームと同様に、入力フィールドに <VueSpecific>`v-model`</VueSpecific><ReactSpecific>`onChange` ハンドラー </ReactSpecific><SvelteSpecific>`bind:`</SvelteSpecific>を追加する必要はありません。各入力に `name` 属性 <ReactSpecific>および（該当する場合は）`defaultValue` </ReactSpecific>を指定するだけで、`Form` コンポーネントがデータ送信を処理してくれます。
</ClientSpecific>
