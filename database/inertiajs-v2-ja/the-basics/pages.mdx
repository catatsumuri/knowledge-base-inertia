...
上記のページがある場合、コントローラまたはルートから [Inertia レスポンス](/v2/the-basics/responses) を返すことで、そのページをレンダリングできます。この例では、このページが Laravel アプリケーション内の Vueでは`resources/js/Pages/User/Show.vue` Reactでは `resources/js/Pages/User/Show.jsx` Svelteでは `resources/js/Pages/User/Show.svelte` に保存されていると仮定します。
...
<ClientSpecific>
ご覧のとおり、このテンプレートには Inertia 固有のものは何も含まれていません。これは単なる通常の Vue, React, Svelte コンポーネントです。
</ClientSpecific>
